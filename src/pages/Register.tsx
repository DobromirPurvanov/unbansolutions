...
export default function Register() {
  ...
  return (
    <>
      ...
      <main className="min-h-[100dvh] flex items-center justify-center pt-16 pb-8 relative overflow-hidden bg-gradient-to-br from-purple-50 via-white to-blue-50">
        <div className="absolute top-0 left-0 w-[300px] h-[300px] bg-purple-200/30 rounded-full filter blur-[80px]" />
        <div className="absolute bottom-0 right-0 w-[250px] h-[250px] bg-blue-200/30 rounded-full filter blur-[60px]" />
        <div className="relative w-full max-w-[360px] mx-auto px-6">
          ...
          <div className="glass-card p-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" className="w-full glow-btn text-xs py-2.5">{t('rp.submit')}</button>
            </form>
          </div>
          <p className="text-center text-slate-700 text-xs mt-6">{t('rp.hasAccount')} <Link to="/login" className="text-blue-700 hover:text-blue-900 transition-colors font-bold">{t('rp.login')}</Link></p>
        </div>
      </main>
    </>
  );
